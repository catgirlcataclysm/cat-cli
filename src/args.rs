...
pub struct Args {
    ...
    #[arg(
        long, 
        short = 'u',
        help = "Don't print the URL of the image",
    )]
    pub nourl: bool,
    ...
    #[arg(
        long, 
        short = 'r',
        help = "Don't print the resolution of the image",
    )]
    pub nores: bool,
...
pub async fn handle_args(img: &ImgData) {
    ...
    crate::img::ImgData::write_img(&img.url, args.width, args.height, args.noimg, args.output).await.expect("Failed printing to the terminal.");
     
    if args.nourl & !args.nores {
        if args.nocolor {
            println!("{} {}x{}", "Resolution:".bold(), img.width.to_string(), img.height.to_string());
        } else {
            println!("{} {}{}{}", "Resolution:".green().bold(), img.width.to_string().blue(), "x".blue(), img.height.to_string().blue());
        }
